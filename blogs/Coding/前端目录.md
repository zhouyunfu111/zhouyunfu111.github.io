...
文章涉及的内容可能不全面，但量很多，需要慢慢看。来源于各个地方，我花了很长的时间整理，希望对大家有帮助。
...
1. block:div等容器类型
1. inline:img span等行内类型
1. table系列：将样式变成table类型
1. flex:重点把握，非常强大
1. vgrid:同上
1. inline-block:可设置宽度，两者间有一点间隙
1. inherit:继承父级
...
![](https://user-gold-cdn.xitu.io/2018/9/9/165bd6dedf60ecfd?w=897&h=529&f=jpeg&s=111439)

如图 1,2,3请求行，4请求体，5请求体

**跨域、JSONP 、CORS、postMessage**

跨域概念解释：当前发起请求的域与该请求指向的资源所在的域不一样。这里的域指的是这样的一个概念：我们认为若协议 + 域名 + 端口号均相同，那么就是同域。 如下表 图
![](https://user-gold-cdn.xitu.io/2018/9/9/165bd6dede8a98d1?w=532&h=203&f=png&s=12558)
...
1. HTTP2.0压缩消息头
1. HTTP2.0服务端推送
1. HTTP2.0只适用于HTTPS的场景

## Vue相关

## 生命周期顺序

![](https://user-gold-cdn.xitu.io/2018/9/9/165bd6dede933ba6?w=800&h=540&f=png&s=146684)
组件通信
...
1. watch像事件监听，对象发生变化时，执行相关操作
1. methods与js中执行方法类似
1. computed通常只有get属性
1. 数据变化的同时进行异步操作或者是比较大的开销，那么watch为最佳选择
1. watch的对象必须事先声明
## 算法相关
各种排序实现
相关数据 ![表格](https://user-gold-cdn.xitu.io/2018/9/9/165bd6dedf755d33?w=700&h=426&f=png&s=165000)
``` js
...
1. DNS解析 先本地缓存找，在一层层找 将常见的地址解析成唯一对应的ip地址基本顺序为：本地域名服务器->根域名服务器->com顶级域名服务器依次类推下去,找到后记录并缓存下来如www.google.com为
